...
//                    }
    ...
    public function leave(Request $request) {
        ...
        $parse_signed_request = function ($signed_request) use ($base64_url_decode) {
            list($encoded_sig, $payload) = explode('.', $signed_request, 2);

            $secret = \FacebookSdk::getApp()->getSecret(); // Use your app secret here
        ...
        try {
            ...
            $userInfo = AdAuth::where(['user_id' => $user_id, 'app_id' => \FacebookSdk::getAppId(), 'type' => 1])->where('status', 0)->first();
            if (empty($userInfo)) {
                return response()->json(['error' => 'user no found']);
            }

            // Start data deletion
            $confirmation_code = md5($signed_request); // unique code for the deletion request
            \Cache::put('remove.user_id.' . $confirmation_code, ['user_id' => $user_id, 'type' => 'leave'], 600);

            $status_url = redirect(route('facebook_deletion', ['sign' => $confirmation_code])); // URL to track the deletion
...
//            }
    ...
    public function remove(Request $request) {
        ...
        try {
            ...
            $data = $parse_signed_request($signed_request);
            $user_id = $data['user_id'];
            $userInfo = AdAuth::where(['user_id' => $user_id, 'app_id' => \FacebookSdk::getAppId(), 'type' => 1])->where('status', '<>', 2)->first();
            if (empty($userInfo)) {
                return response()->json(['error' => 'user no found']);
            }
            ...
            // Start data deletion
            $confirmation_code = md5($signed_request); // unique code for the deletion request
            \Cache::put('remove.user_id.' . $confirmation_code, ['user_id' => $user_id, 'type' => 'remove'], 600);
...
//            }
    ...
    public function deletion(Request $request)
    {
        ...
        $info = \Cache::get('remove.user_id.' . $sign, 0);
        if (empty($info) || empty($info['user_id']) || empty($info['type'])) {
            return 'no found data';
        }
        $user_id = $info['user_id'];
        if ('remove' == $info['type']) {
            AdAccount::where('user_id', $user_id)->where('app_id', \FacebookSdk::getAppId())->where('type', 1)->update(['status' => 2]);
            AdAuth::where('user_id', $user_id)->where('app_id', \FacebookSdk::getAppId())->where('type', 1)->update(['status' => 2]);

            echo "act account deleted, user info deleted ";
        } elseif ('leave' == $info['type']) {
            AdAccount::where('user_id', $user_id)->where('app_id', \FacebookSdk::getAppId())->where('type', 1)->update(['status' => 1, 'access_token' => '']);
            AdAuth::where('user_id', $user_id)->where('app_id', \FacebookSdk::getAppId())->where('type', 1)->update(['status' => 1, 'access_token' => '']);
            echo "act account token invalid, user info invalid ";
        }
